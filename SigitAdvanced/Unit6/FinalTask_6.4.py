...
def draw_connected_dots(coords, image_size):
    """
    Draws lines connecting the given coordinates on a new transparent image.

    Args:
        coords: A tuple containing x, y coordinates in the format of (x1, y1, x2, y2, ...).
        image_size: A tuple containing the width and height of the image.

    Returns:
        A new transparent image with lines connecting the given coordinates.
    """
...
def combine_images(base_img, overlay_img):
    """
    Combines a base image with an overlay image.

    Args:
        base_img: The base image.
        overlay_img: The overlay image to be combined with the base image.

    Returns:
        A new image with the overlay image combined on top of the base image.
    """
    return Image.alpha_composite(base_img.convert('RGBA'), overlay_img)


def main():
    base_img = Image.open("img.jpg")
    width, height = base_img.size

    first_img = draw_connected_dots(first, (width, height))
    second_img = draw_connected_dots(second, (width, height))

    combined_img = combine_images(base_img, first_img)
    combined_img = combine_images(combined_img, second_img)
